...
\section{Pre-Processing Citra}
    Pada tahap ini citra ikan akan melalui tiga tahapan penghapusan background, rotasi gambar, \emph{smoothing gaussian} untuk memperhalus \emph{feature}, dan \emph{Perspective Projection}.~Tahapan ini bertujuan untuk mempersiapkan citra untuk diproses lebih lanjut.

% \subsection{Tahap Pengambilan Gambar}
\subsection{Penghapusan Background}
    \emph{Dataset} diambil sebuah penangkaran ikan menggunakan kamera \emph{smartphone} dengan resolusi 12 MP.\@ Gambar diambil pada siang hari dengan pencahayaan alami yang cukup untuk memastikan kualitas gambar yang baik. 
    Ikan diletakkan di atas conveyor belt. Setiap gambar diambil dari sudut pandang yang konsisten, yaitu \emph{TopDown} dengan jarak 45 centimeter dari Ikan, untuk memastikan bahwa seluruh bagian ikan terlihat jelas dalam gambar. Gambar-gambar ini kemudian disimpan dalam format JPEG atau PNG untuk digunakan sebagai input dalam sistem penghitungan panjang dan berat ikan.
...
% \subsection{Penghapusan Background}
    Penghapusan background akan menggunakan library \emph{\textbf{rembg}}. Proses penghapusan background dilakukan dengan cara menginput gambar yang akan diproses ke dalam library \emph{rembg}, hasil outputnya berupa gambar yang telah dihilangkan latar belakangnya dan digantikan dengan warna solid putih sebagai latar belakang penggantinya.
    \begin{figure}[H]
	\centering
	\begin{subfigure}{.3\textwidth}
		\centering
...
\subsection{Rotasi}
    Rotasi dilakukan untuk menyesuaikan orientasi gambar agar sesuai dengan posisi ikan yang diinginkan. Proses rotasi ini penting untuk memastikan bahwa titik-titik sudut yang akan dideteksi berada dalam posisi yang optimal untuk penghitungan panjang dan berat ikan. Rotasi dilakukan dengan menggunakan PCA (Principal Component Analysis) untuk menentukan arah utama dari objek ikan dalam gambar, sehingga gambar dapat diputar agar sumbu utama objek sejajar dengan sumbu horizontal atau vertikal. 
    Dengan demikian, proses deteksi sudut dan penghitungan panjang serta berat ikan dapat dilakukan dengan lebih akurat dan konsisten.
    \begin{figure}[H]
        \centering
        \begin{subfigure}{.3\textwidth}
            \centering
...
\section{Penghitungan panjang dan berat}
...
\subsection{Melakukan Proyeksi Perspektif}
    ...
    Dimana \(x'\) dan \(y'\) adalah panjang dan lebar ikan yang telah dikoreksi dengan proyeksi perspektif dan akan digunakan untuk menghitung berat ikan pada tahap selanjutnya. 

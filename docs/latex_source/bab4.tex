...
\end{figure}
    ...
    Fungsi \emph{Compute\_Gradient\_X} dan \emph{Compute\_Gradient\_Y} menggunakan kernel Sobel untuk menghitung gradien pada sumbu X dan Y.
...
\section{NMS dan thresholding}
 Nilai Harris yang didapatkan dari fungsi \emph{Harris} masih terlalu banyak jika langsung digunakan untuk mengekstraksi sudut. Perlu dilakukan proses thresholding dan non-maximum suppression untuk mendapatkan titik-titik sudut yang Akurat. Berikut \emph{Source Code} yang digunakan untuk melakukan thresholding dan non-maximum suppression:
\begin{figure}[H]
    \begin{lstlisting}[language=Python, basicstyle=\tiny]
...
Dengan Menggunakan rumus \emph{Mean Absolute Percentace Error} atau MAPE, didapatkan hasil evaluasi akurasi pada table~\ref{tab:MAPE} berikut:
